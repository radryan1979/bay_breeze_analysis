...
This will be primarily to analyze days with classic sb
on one coast coinciding with CWS or DPWS on opposing coast.
...
for cst in ["EC_SB","WC_SB","BC_SB","Non_SB"]:#
    ...
    sm._A = []
    ax.contourf(lons, lats, rain_perc, 70,
                 transform=ccrs.PlateCarree(), norm=norm, cmap=cmap)
    ...
    plt.savefig("/Volumes/LaCie/SeaBreeze/Florida/StationDetection/Results3_6_19/Figures/15HrPrecip/{0}_PercentPrecip.png".format(cst),
        dpi=500)
    plt.show()
