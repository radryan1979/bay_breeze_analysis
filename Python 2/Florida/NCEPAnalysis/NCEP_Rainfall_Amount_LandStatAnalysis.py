...
for cst in ["EC_SB","WC_SB","BC_SB","Non_SB"]:#
    ...
    for date in dates:
        ...
        if int(SBF_time.month)!=mo:
            ...
            if mo==6:
                perc_sbdays[j]=mocnt/30.0*100
            else:
                perc_sbdays[j]=mocnt/31.0*100
    ...
    if mo==6:
        ...
        perc_sbdays[j]=mocnt/31.0*100
    perc_masdar[j] = np.nansum(monthly_precip)/np.nansum(monthly_accum)*100
    np.savetxt(NCEPdirpath+"yrly_percMASDAR_{0}_Land.csv".format(cst),perc_masdar,delimiter=',')
    
    print(cst)
    if cst=="EC_SB":
        print(chisquare(perc_masdar[1:], f_exp=perc_sbdays[1:]))
    else:
        print(chisquare(perc_masdar, f_exp=perc_sbdays))
