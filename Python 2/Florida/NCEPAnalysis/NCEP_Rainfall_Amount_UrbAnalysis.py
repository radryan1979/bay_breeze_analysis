...
agBinarypath = "/Volumes/LaCie/SeaBreeze/Florida/NCEP4kmGridSubset/NCEP_FL_UrbBinary.csv" #Created using ArcMap
NCEP_ag = pd.read_csv(agBinarypath)
NCEP_ag = NCEP_ag.drop(["FID","OBJECTID_1","Join_Count","TARGET_FID","F1","OBJECTID"],axis=1)
ag_binary = np.array(NCEP_ag["Urb_Binary"]).reshape(lats.shape[0],lats.shape[1])
...
for cst in ["BC_SB"]:#"EC_SB","WC_SB","BC_SB","Non_SB"]:#
    
    perc_masdar=np.empty(33)
    j=0
    ...
    for date in dates:
        ...
        if int(SBF_time.month)!=mo:
            # np.savetxt(NCEPdirpath+"{0}_yearly_accum_{1}.csv".format(cst,yr),yearly_precip,delimiter=',')
            monthly_accum = np.loadtxt(NCEPdirpath+"total_yearly_accum_{0}_{1}.csv".format(mo,yr),dtype=bytes,delimiter=',')
            monthly_accum = monthly_accum.astype(float)
            
            monthly_accum = np.multiply(monthly_accum,ag_binary)
            
            
            print(yr)
            
            perc_masdar[j] = np.nansum(monthly_precip)/np.nansum(monthly_accum)*100
            
            mo = int(SBF_time.month)
            del monthly_precip
            mocnt=0
            j+=1
            
            if int(SBF_time.year)!=yr:
                yr=int(SBF_time.year)
    ...
    print(yr)
    perc_masdar[j] = np.nansum(monthly_precip)/np.nansum(monthly_accum)*100
    np.savetxt(NCEPdirpath+"yrly_percMASDAR_{0}_Urban.csv".format(cst),perc_masdar,delimiter=',')
"""    
    ...
    if ag_analysis:
        ...
        print("{0} days make up {1} of total days.\n The percent of daytime precipitation over urban land on {0} days is {2}.".format(cst,len(dates)/1012.0*100,perc_ag_precip))
    ...
    if ag_analysis:
        ...
        plt.savefig("/Volumes/LaCie/SeaBreeze/Florida/StationDetection/Results3_6_19/Figures/UrbPrecip15Hr/SBC_PoP_{0}_Classic.png".format(cst), 
            dpi=500)
        plt.close()
        ...
        ax.add_feature(coast, linestyle="-", edgecolor="darkslategray", linewidth=1)
        ax.set(xlim=(-83.75,-79.75),ylim=(24.4,30.5))
        plt.savefig("/Volumes/LaCie/SeaBreeze/Florida/StationDetection/Results3_6_19/Figures/UrbPrecip15Hr/{0}_PrecipAccum.png".format(cst),
            dpi=500)
        plt.close()
        ...
        ax.add_feature(coast, linestyle="-", edgecolor="darkslategray", linewidth=1)
        ax.set(xlim=(-83.75,-79.75),ylim=(24.4,30.5))
        plt.savefig("/Volumes/LaCie/SeaBreeze/Florida/StationDetection/Results3_6_19/Figures/UrbPrecip15Hr/{0}_Classic_PercentPrecip.png".format(cst),
            dpi=500)
        plt.close()
        ...
        plt.xlim(0)
        plt.savefig("/Volumes/LaCie/SeaBreeze/Florida/StationDetection/Results3_6_19/Figures/UrbPrecip15Hr/DailyPrecip_{0}_Distribution.png".format(cst),
            dpi=500)
        plt.close()
