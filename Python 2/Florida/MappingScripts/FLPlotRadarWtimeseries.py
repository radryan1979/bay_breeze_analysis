...
def trim_rad_data2(lats, lons, ref, boundinglat=0, boundinglon=0):
    for i in range(lats.shape[0]):
        for j in range(lats.shape[1]):
            if (lats[i][j]>boundinglat[0] and lats[i][j]<boundinglat[1] and \
                lons[i][j]>boundinglon[0] and lons[i][j]<boundinglon[1]):
...
stations=np.array([420])#,440,425,410,450])
...
query=rs.query()
date1=datetime(2015,4,27,19,15)
date2=datetime(2015,4,27,22)
query.stations('KAMX').time(date1)
...
query.stations('KAMX').time(date2)
rs.validate_query(query)
...
new_ref = gref2*miami_mask[:,:,0]+gref2*miami_mask[:,:,1]+gref2*miami_mask[:,:,2]
...
df = pd.read_csv('/Users/dpmoore2927/Desktop/UD/Hyperion/4_27_15_Hyperion_CaseStudy.csv', 
    header=0,index_col=0,parse_dates=True)
df = df.rename(columns={"temp_air_10m_C":"Temp (\N{DEGREE SIGN}C)",
                        "wind_speed_10m_mph":"Wind Speed (mph)",
                        "wind_direction_10m_deg":"Wind Direction (\N{DEGREE SIGN})"
                        })
...
ax1.set_title('KAMX NEXRAD {0}'.format(date1))
ax1.set(aspect=1,
...
for st in stations:
    plt.plot(locs.loc[st][3],locs.loc[st][2], '*',
    markersize=22, markerfacecolor=(0,1,1,1),
    markeredgewidth=2, markeredgecolor='k',label='FAWN Station 420')
...
plt.xticks(np.arange(-80.0,-81.3,-0.25))
plt.yticks(np.arange(25.1,26.7,0.25))
...
ax2.set_title('KAMX NEXRAD {0}'.format(date2))
ax2.set(aspect=1,
        # xlim=(glon[0][0],glon[0][-1]),
        # ylim=(glat[0][1],glat[-1][0]))
        xlim = (-81.25, -80.0),
...
# fig.title('April 27, 2015 Sea Breeze')

ax_inset = fig.add_axes([0.105,0.24,0.14,0.14], projection=ccrs.PlateCarree())
ax_inset.set_global()
...
ax_inset.add_patch(mpatches.Rectangle(xy=[-81.25, 25.1], width=1.3, height=1.6,
...
fig.suptitle('April 27, 2015 Sea Breeze',fontsize=24,y=0.85)
...
df['Wind Speed (mph)'].plot(color='g',sharex=my_ax1,)
my_ax2.set_ylabel('Wind Speed (mph)',rotation='vertical')
...
fig2.patches.extend([plt.Rectangle((0.55,0.061),0.02,0.68,
...
fig2.legend(handles=legend_elements,loc=(0.8,0.11))
