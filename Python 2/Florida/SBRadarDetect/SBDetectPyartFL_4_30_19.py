...
def sb_day_analysis(rs, date):
    ...
    query=rs.query()
    query.stations('KMLB').time_range(dt,nextday)
    ...
    if len(ds)==0:
        ...
        return day_sb_found, error, sbf_lon_day, scans_df, scan_time
    ...
    for number in range(13,24):
        ...
        #If next iteration is within 1 hour of previous iteration, this scan is skipped.
        # if scan_num>0 and timediff<1.0:
        #     continue
    ...
    ###########################################################################
        ...
        if np.count_nonzero(unmasked[unmasked>20.0])/\
        np.count_nonzero(unmasked) >= 0.10:
            sbf_lon_day.append([np.nan]*len(cstlnlon))
            print('Error, too much precip (>20% of study area), scan skipped.')
            continue
    ...
    ###########################################################################
        ...
        if lats.max()>100.0 or lons.max()>100.0:
            #Erroneous value in lats/lons, skip scan.
...
def regrid_to_cartesian(radar, lon0, lat0):
    ...
    lat_shape = [lats.shape[0]-1,lats.shape[1]-1]
    lon_shape = [lons.shape[0]-1,lons.shape[1]-1]
        
    #Otherwise, lats and lons have 1 extra data point.
    lats = lats[0:lat_shape[0], 0:lat_shape[1]]
    lons = lons[0:lon_shape[0], 0:lon_shape[1]]
    return lons,lats
    ...
    for i in range(np.size(cstlnlat)):
        ...
        for j in range(idxlon,idxlonend,-1):
            ...
            if sumref>=80:
                ...
                ave_bg_refW = np.nanmean(gref[idxlat-5:idxlat+5,j-25:j-5])
                
                ave_bg_refE = np.nanmean(gref[idxlat-5:idxlat+5,j+5:j+25])
...
loref = 8.0; hiref = 25.0 #dBz thresholds
...
w_dir = "/home/1898/RadSBDetect/SBDetectFL4_30_19/"
...
for i in range(2018,2019):
    ...
    for j in range(6,9):
        ...
        for k in range(1,32):
            ...
            sb_found_bool, error, sbf_lon, scans_df, scan_time = sb_day_analysis(rs, date)
