...
Purpose: Create feather plots of wind data for up to two stations.
...
out_path            =       "/Volumes/LaCie/SeaBreeze/Florida/StationDetection/Results1_17_19/Figures/WindClimatology/{0}/".format(test_st)
...
for reg in range(1,10):
    ...
    fig, ax = plt.subplots(1,1, figsize=(15,4))
    ax.quiver(regime_hrly_wd_ref.index, np.ones(len(regime_hrly_wd_ref)) * 1.5, 
                                        ...
                                        color='b', headwidth=4, headlength=4, width=0.0025, label="Reference")
    ax.quiver(regime_hrly_wd_test.index, np.ones(len(regime_hrly_wd_test)) * 1.5, 
    ...
    plt.xlabel("Eastern Time")
    plt.legend()
    plt.title("Florida Regime {0} Wind Climatology for \nTest Station: {1} and Reference Station: {2}".format(reg, test_st, ref_st))
    plt.savefig(out_path+"{0}vs{1}Regime{2}Wind.png".format(test_st,ref_st,reg),dpi=500)
    plt.show()