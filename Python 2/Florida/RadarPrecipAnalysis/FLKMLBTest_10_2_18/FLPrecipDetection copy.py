...
def detect_convect(rs, rad, date):
...
for i in range(np.shape(miami_mask)[0]):
    for j in range(np.shape(miami_mask)[1]):
        if ((i>=205 and i<510) and (j>=45 and j<118)) or \
            ((i>=100 and i<205) and (j>=75 and j<148)) or \
            ((i>=50 and i<100) and (j>=110 and j<205)):
                miami_mask[i,j,0] = 1.0

#Set east_2 mask            
for i in range(np.shape(miami_mask)[0]):
    for j in range(np.shape(miami_mask)[1]):
        if ((i>=205 and i<510) and (j>=118 and j<210)) or \
            ((i>=100 and i<205) and (j>=148 and j<210)):
                miami_mask[i,j,1] = 1.0
#Set west_1 mask
    #bounding points (i,j):NW(460,450),SW(100,300),NE(460,355),SE(100,205)
#Slope of lines
m1 = 360.0/150.0
#"y-intercept" for each bounding line
b_west1 = 460.0 - m1 * 450.0
b_east1 = 460.0 - m1 * 355.0
for i in range(np.shape(miami_mask)[0]):
    for j in range(np.shape(miami_mask)[1]):
        if ((i>=40 and i<100) and (j>=200 and j<295)) or \
            ((i>=100 and i<460) and (j<((i-b_west1)/m1) and \
            j>((i-b_east1)/m1))):
                miami_mask[i,j,2] = 1.0

#Set Tampa mask (West_1, West_2, West_3)
    #West_1
    #bounding points (i,j):NW(480,294),NE(480,237),SW(24,87),SE(24,30)

# tampa_west_1 = np.zeros((500,500))
# for i in range(np.shape(miami_east_1)[0]):
#     for j in range(np.shape(miami_east_1)[1]):
        
# tampa_west_2 = np.zeros((500,500))
# for i in range(np.shape(miami_east_1)[0]):
#     for j in range(np.shape(miami_east_1)[1]):
        
# tampa_west_3 = np.zeros((500,500))
# for i in range(np.shape(miami_east_1)[0]):
#     for j in range(np.shape(miami_east_1)[1]):
...
mlb_mask = np.zeros((500,500,2))
    ...
    #bounding points (i,j):NW(480,294),NE(480,237),SW(24,87),SE(24,30)
m3 = 456.0/207.0
b_west5 = 480.0 - m3 * 294.0
b_east5 = 480.0 - m3 * 237.0
for i in range(np.shape(mlb_mask)[0]):
    ...
    #East_2
    #bounding points (i,j):NW(480,361),NE(480,294),SW(24,154),SE(24,87)     
b_east6 = 480.0 - m3 * 294.0
b_west6 = 480.0 - m3 * 361.0
for i in range(np.shape(mlb_mask)[0]):
    for j in range(np.shape(mlb_mask)[1]):
...
#Ingest date file:
df = pd.read_csv('/Users/dpm/Desktop/Hyperion/HughesEastTimes_TestKMLB.csv')
...
for i in df.index:
    ...
    if df['Station'][i] in miami_st:
        ...
        #Just for now, to focus on Miami:
        continue
    ...
    convect_bool, error, df_temp = detect_convect(rs, rad_st, date)
    
    convection.append(convect_bool); error_log.append(error)
    ref_df = ref_df.append(df_temp)

ref_df.to_csv("/Users/dpm/Desktop/Hyperion/RefBins.csv")
