...
def detect_convect(rs, rad, date):
    ...
    for numrad in range(0,len(ds),2):
        ...
        if np.count_nonzero(~np.isnan(my_ref)) < 1:
            continue
        else:
            convect_found=True
...
for i in range(np.shape(miami_mask)[0]):
    for j in range(np.shape(miami_mask)[1]):
        if ((i>=205 and i<510) and (j>=382 and j<455)) or \
...
for i in range(np.shape(miami_mask)[0]):
    for j in range(np.shape(miami_mask)[1]):
        if ((i>=205 and i<510) and (j>=290 and j<382)) or \
            ((i>=100 and i<205) and (j>=290 and j<352)):
                miami_mask[i,j,1] = 1.0
#Set west_1 mask
    #West bounding line:
        #bounding points (i,j): NW(460,50)
m1 = 360.0/150.0
b_west = 460.0 - m1 * 50.0
b_east = 460.0 - m1 * 145.0
for i in range(np.shape(miami_mask)[0]):
    for j in range(np.shape(miami_mask)[1]):
        if ((i>=40 and i<100) and (j>=200 and j<295)) or \
            ((i>=100 and i<460) and (j>((i-b_west)/m1) and \
            j<((i-b_east)/m1))):
                miami_mask[i,j,2] = 1.0
        ####FIX THIS

# tampa_east_1 = np.zeros((500,500))
# for i in np.shape(miami_east_1)[0]:
#     for j in np.shape(miami_east_1)[1]:
...
# mlb_east_1 = np.zeros((500,500))
# for i in np.shape(miami_east_1)[0]:
#     for j in np.shape(miami_east_1)[1]:
        
# mlb_east_2 = np.zeros((500,500))
# for i in np.shape(miami_east_1)[0]:
#     for j in np.shape(miami_east_1)[1]:
