...
Update 6-24-19: Updated thredds server location.

Update 7-2-19: Updated to use AWS server (nexradaws) as
pyart is failing to access server at unidata.

Analyzes KDOX Radar in Florida.

This program will look hourly to fill gaps in thesis analysis
will need to be updated for conserve remap!
...
def sb_day_analysis(rs, date):
    ...
    #Get scans for day
    try:
        scans = conn.get_avail_scans_in_range(dt,nextday,'KMLB')
    ...
    for number in range(0,4):
        ...
        try: 
            ...
            for ijk in range(len(results.success)):
                # if top_of_hour:
                if date.year<2016.0:
                    if str(results.success[ijk])[-14:-12]==str(number).zfill(2):
                        ds_use = results.success[ijk]
                        # top_of_hour=False
                        break
                else:
                    if str(results.success[ijk])[-11:-9]==str(number).zfill(2):
                        ds_use = results.success[ijk]
                        # top_of_hour=False
                        break
                # else:
    ...
    ###########################################################################
        ...
        NLatbgbox = find_nearest(onedimlat, 28.7)
        SLatbgbox = find_nearest(onedimlat, 27.5)
        ELonbgbox = find_nearest(onedimlon, -79.8)
        WLonbgbox = find_nearest(onedimlon,-81.4)
...
def conserve_remap_to_cartesian(radar, lon0, lat0):
    ...
    regridder = xe.Regridder(grid_in, out_grid, 'bilinear')
...
# lon_out = out_grid['lon'].values
# lat_out = out_grid['lat'].values
...
#"https://thredds.ucar.edu/thredds/catalog/nexrad/level2/"
# rs=RadarServer('http://thredds-test.unidata.ucar.edu/thredds/catalog/nexrad/level2/')

conn = nexradaws.NexradAwsInterface()

w_dir = "/home/1898/RadSBDetect/SBDetectAWSFL7_2_19/"

#Temp location for scans to download then be deleted
temp_loc = w_dir+"temp_scans/"
...
for i in range(2009,2019):
    ...
    for j in range(6,9):
        ...
        for k in range(1,32):
            ...
            areal_precip_error = sb_day_analysis(conn, date)
