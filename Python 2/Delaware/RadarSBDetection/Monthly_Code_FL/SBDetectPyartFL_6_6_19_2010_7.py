...
Update 6_6_19: Updated to SciPy regrid method to conserve
memory usage. Hopefully prevent crash. Restart in 2010, 
as 2009 is almost complete.
...
def conserve_remap_to_cartesian(radar, lon0, lat0):
    ...
    data_in = ds['reflectivity'].values
    grid_in = xe.backend.esmf_grid(lon_in.T, lat_in.T)
    grid_out = xe.backend.esmf_grid(lon_out.T, lat_out.T)
    
    filename = 'test_weights.nc' # weight filename
    if os.path.exists(filename):
        os.remove(filename)
    
    regrid = xe.backend.esmf_regrid_build(grid_in, grid_out, 'bilinear',
            filename=filename)
    ...
    return data_out, out_grid.lon, out_grid.lat
...
w_dir = "/home/1898/RadSBDetect/SBDetectFL5_15_19/"
