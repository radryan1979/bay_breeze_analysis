...
def sb_day_analysis(rs, date):
    ...
    #Number of scans properly analyzed
    scan_num = 0
    ...
    ###########################################################################
        ...
        #Test:
        print(timestamp, sbfound, sbf, datetime.now())
...
def first_pass_sea_breeze_front_coords(gref, glon, glat):
    ...
    idxlonend = find_nearest(onedimlon,-75.75)
    ...
    for i in range(np.size(cstlnlat)):
        ...
        for j in range(idxlon,idxlonend,-1):
            ...
            sumref = np.count_nonzero(~np.isnan(gref[idxlat-8:idxlat+8,j-5:j+5]))
            
            #If larger cluster found, replace previous.
            if sumref>=120:
                print("sum ref", sumref)
                print("Point found:",cstlnlat[i],onedimlon[j])
                print("distance to coast: {0}.".format(onedimlon[idxlon]-onedimlon[j]))
                #Added conditional to be within 0.25degrees of coastline - empirical.
                if sumref>maxsumref and (onedimlon[idxlon]-onedimlon[j])<0.25:
                    print("success, sbf point found.")
                    maxsumref = sumref
                    maxrefj = j
    ...
    if float(sbfcnt)/float(len(cstlnlat))>0.34:
        ...
        for i in range(len(sbf_idx_list)-1):
            ...
            for j in range(sbf_idx_list[i],sbf_idx_list[i+1]):
...
def sea_breeze_front_coords(gref, glon, glat, prev_lon):
    ...
    #Average of Previous coordinates
    prev_lon_av = np.mean(prev_lon[-30:])
    ...
    for i in range(np.size(cstlnlat)):
        ...
        for j in range(idxlon,idxlonend,-1):
            ...
            if sumref>=100:
                print("Point found:",cstlnlat[i],onedimlon[j])
                #Added conditional to be within 0.3degrees of the average 
                #of all sbf coordinates.
                if sumref>maxsumref and np.abs(prev_lon_av-onedimlon[j])<0.3:
                    maxsumref = sumref
                    maxrefj = j
...
for i in range(2016,2017):
    ...
    for j in range(8,9):
        ...
        if i==2013 and j==5:
            ...
            df.to_csv("/home/1898/RadSBDetect/{0}_{1}_RadarDetect.csv".format(i,j))
...
total_df.to_csv("/home/1898/RadSBDetect/2013_2017RadarDetect.csv")
