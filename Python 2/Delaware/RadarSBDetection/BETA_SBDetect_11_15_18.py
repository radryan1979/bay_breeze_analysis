...
def sb_day_analysis(rs, date):
    ...
    if rs == '':
        radar = pyart.io.read_cfradial('/Users/dpmoore2927/Desktop/2016_8_19_CaseStudy.nc')
...
def plot_sbf(date, gref, glon, glat, sbf=()):
    ...
    plt.savefig('/Users/dpmoore2927/Desktop/test.png')
    return
...
def connected_areas(gref2):
    ...
    for i in range(len(areas)):
        ...
        if areas[i]>200:
            print(areas[i],perimeters[i],labels[i],eccentricity[i],orientation[i])
            print(p_a_ratio)
        ...
        if areas[i]>500 and p_a_ratio>0.20 and means[i]<20.0 and \
