...
def sb_day_analysis(rs, date):
    ...
    ###########################################################################
        if num_sb_found == 0:
            sbfound, sbf = first_pass_sea_breeze_front_coords(gref,glon,glat,rad_mode)
        else:
            #If SBF has been found previously on this day, use previous coordinates
            #to minimize error of erroneous SBF coordinates.
            sbfound, sbf = sea_breeze_front_coords(gref,glon,glat,sbflon,rad_mode)
...
#one is not found.
def first_pass_sea_breeze_front_coords(gref, glon, glat, rad_mode):
    ...
    for i in range(np.size(cstlnlat)):
        ...
        for j in range(idxlon,idxlonend,-1):
            ...
            if sumref>=60:
                ...
                ave_bg_ref = np.nanmean(gref[idxlat-10:idxlat+10,j-10:j+10])
...
#Calculate sea breeze front coordinates, raise warning if 
#one is not found.
def sea_breeze_front_coords(gref, glon, glat, prev_lon, rad_mode):
    #For easier processing:
    onedimlat = glat[:,0]
    onedimlon = glon[0,:]
...
for i in range(2013,2018):
    ...
    for j in range(5,10):
        ...
        if i==2013 and j==5:
            ...
            df.to_csv("/Volumes/LaCie/SeaBreeze/RadarDetectOutput/{0}_{1}_RadarDetect.csv".format(i,j))
...
total_df.to_csv("/Volumes/LaCie/SeaBreeze/RadarDetectOutput/2013_2017RadarDetect.csv")
