"""
Coded by Dan Moore
This program will ingest a user input datetime, analyze the radar imagery
in ~15 min increments and determine whether a sea breeze was detected or not.
It will also print out a csv to the local directory of sea breeze front 
positions for each scan time that a sea breeze front is successfully detected
for post-processing.
...
print("This program will ingest a user input date, analyze radar scans \
    in roughly 15 minute intrements to detect the presence and propagation \
    of a sea breeze front along the Eastern Coast of Delaware. \
    The result will be a screen output of either 'Sea Breeze Detected' \
    or 'Sea Breeze Not Detected'. If a sea breeze was detected, a .csv \
    file will also be created in the local directory containing coordinates \
    of the sea breeze front at each radar scan.")
...
if sb_found_bool:
    ...
    df.to_csv("SBF_Coords_{0}.csv".format(date_string))
