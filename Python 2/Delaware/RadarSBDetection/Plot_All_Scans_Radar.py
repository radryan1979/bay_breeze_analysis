...
def sb_day_analysis(rs, date, station):
    ...
    if rs == '':
        ...
        dt = date + timedelta(hours=15) + timedelta(minutes=23)
        
        try:
            scans = conn.get_avail_scans_in_range(dt,nextday,station)
        except:
        ...
        results = conn.download(scans,w_dir)
    ...
    for i, scan in enumerate(results.iter_success(),start=1):
        ...
        if first_scan:
            ...
            if (timestamp-prev_time).seconds/60.0<15.0:
                continue
        prev_time = timestamp
...
def plot_sbf(date, gref, glon, glat, title, sbf=(), sbfinterp=()):
    ...
    if title == "plot1" or title == "plot2" or title == "plot3":
        title=str(date.date())+"_"+str(date.time())
        lines=False
    else:
        lines=True
    ...
    reader = shpreader.Reader('/home/1898/Shapefiles/countyl010g_shp_nt00964/countyl010g.shp')
...
w_dir = "/home/1898/RadSBDetect/plot_scans/"
...
date = datetime(2012,7,3,00,00)

sb_found_bool, sbf = sb_day_analysis(conn, date, station)
