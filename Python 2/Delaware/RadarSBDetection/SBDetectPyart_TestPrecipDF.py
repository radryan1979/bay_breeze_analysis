...
def sb_day_analysis(rs, date):
    ...
    ###########################################################################
        ...
        gref, sb_precip, precip_df = connected_areas(gref)
        ...
        if sb_precip:
            precip_df['Location'] = rowcol_to_latlon(precip_df['Location'], lats, lons)
...
def connected_areas(gref2):
    ...
    for i in range(len(areas)):
        ...
        if areas[i]>100 and means[i]>25.0:
        ...
        if areas[i]>750 and p_a_ratio>0.20 and means[i]<25.0 and \
...
def rowcol_to_latlon(rowcol,lats,lons):
    ...
    latlon = ()
    for rc in rowcol:
        latlon.append((lats[rc[0]],lons[rc[1]]))
