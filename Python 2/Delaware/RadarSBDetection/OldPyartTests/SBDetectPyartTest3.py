...
The Purpose of this version is to develop a method of eliminating storm
cells from the data. We will attempt to eliminate storm cells by 
thresholding the velocity of points over a certain reflectivity value.
This will be done prior to the data being sent to the detection algorithm
such that the detection algorithm, sea_breeze_front_coords receives an 
altered matrix.
...
def sb_day_analysis(rs, date):
    ...
    ###########################################################################
        ...
        vel = radar.get_field(0, 'velocity')
        unmasked = ma.getdata(ref)  
        unm_vel = ma.getdata(vel)
        ...
        my_vel = trim_rad_data(lats, lons, unm_vel, boundinglat, boundinglon)
    ...
    ###########################################################################
        ...
        gvel, _, _ = grid_to_rectilinear(my_vel,lons,lats,nlon,nlat)
...
def trim_rad_data(lats, lons, ref, boundinglat=0, boundinglon=0):
    for i in range(lats.shape[0]):
        for j in range(lats.shape[1]):
            ...
            #Previously had the thresholding of reflectivity here,
            #but the reflectivities will be needed to eliminate
            #precipitation.
            #if (ref[i][j]<loref or ref[i][j]>hiref):
                #ref[i][j] = np.nan
...
#Detect storm systems and eliminate outer edge so that it is invisible
#to the detection algorithm.
def elim_storms(gref,gvel,glon,glat):
    #For easier processing:
    onedimlat = glat[:,0]
    onedimlon = glon[0,:]
    
    while np.nanargmax(gref)>25.0:
    
        #Find location of highest reflectivity value in entire array:
        idx = np.nanargmax(gref)
        row = idx//len(onedimlon)
        col = idx %len(onedimlon)
        
        #Find points surrounding the highest value that have velocities
        #higher than ________
