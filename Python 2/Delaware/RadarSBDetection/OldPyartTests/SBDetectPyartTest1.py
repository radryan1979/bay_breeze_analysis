"""
Coded by Dan Moore
Test for SBDetectPyart on 9-6-18
...
def sb_day_analysis(rs, date):
    ...
    ###########################################################################
        ...
        if sbfound:
            ...
            if num_sb_found>=1:
                ...
                sb_pen_nc.append((sbflon_nc[num_sb_found]-np.mean(cstlnlon[6:12]))*met_per_degree)
                sb_pen_c.append((sbflon_c[num_sb_found]-np.mean(cstlnlon[12:18]))*met_per_degree)
                sb_pen_sc.append((sbflon_sc[num_sb_found]-np.mean(cstlnlon[18:24]))*met_per_degree)
                sb_pen_s.append((sbflon_s[num_sb_found]-np.mean(cstlnlon[24:30]))*met_per_degree)
    ...
    if numrad<20:
        ...
        sb_vel = np.nan
        sb_pen = np.nan
        return day_sb_found, error, sb_vel, sb_pen, scan_time
...
datelist=[datetime(2018,7,2)]#, datetime(2018,7,3)]
...
for i in range(len(datelist)):
    ...
    sb_found_bool, error, sb_vel, sb_pen, scan_time = sb_day_analysis(rs, datelist[i])
    
    sb_found.append(sb_found)
    error_log.append(error)
    
    transect_lats = cstlnlats
