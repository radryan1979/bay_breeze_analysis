"""
Coded by Dan Moore
Test program for messing with algorithm and plotting output.
...
def sb_day_analysis(rs, date):
    ...
    if rs == '':
        radar = pyart.io.read_cfradial('/Users/dpm/Desktop/2017_7_4_CaseStudy.nc')
...
def plot_sbf(date, gref, glon, glat, sbf=()):
    ...
    plt.savefig('/Users/dpm/Desktop/test.png')
    return
...
def connected_areas(gref2):
    ...
    maxs = ndimage.maximum(gref2, labeled, range(ncomponents + 1))
    ...
    for i in range(len(areas)):
        ...
        if areas[i]>100 and means[i]>17.5:
        ...
        if areas[i]>750 and p_a_ratio>0.20 and means[i]<17.5 and \
...
date = datetime(2016,8,19,20,30)

sb_found_bool, sbf = sb_day_analysis(rs, date)
