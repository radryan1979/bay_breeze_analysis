...
def sb_day_analysis(rs, date, station):
    ...
    if rs == '':
        radar = pyart.io.read_cfradial(w_dir+'2016_8_12CaseStudy.nc')
        ...
        print(ds[0].access_urls["OPENDAP"])
        
        # now that we have the data, let's go ahead and open it with pyart
        radar = pyart.io.read_nexrad_cdm(ds.access_urls['OPENDAP'])#,exclude_fields=None,include_fields=["Reflectivity"])
    ...
    sbfound, sbf, sbfinterp = first_pass_sea_breeze_front_coords(gref,glon,glat,ave_bg_ref,
                                                        ave_bg_refmed)#,rad_mode)
    
    print('sbfound =',sbfound)
    
    plot_sbf(timestamp, gref, glon, glat, "plot3", sbf, sbfinterp)
...
#one is not found.
def first_pass_sea_breeze_front_coords(gref, glon, glat,ave_bg_ref,ave_bg_refmed):
    ...
    for i in range(np.size(cstlnlat)):
        ...
        for j in range(idxlon,idxlonend,-1):
            ...
            if sumref>=60:
                ...
                ave_bg_refW = np.nanmean(gref[idxlat-5:idxlat+6,j-10:j-6])
...
rs = TDSCatalog('http://thredds-aws.unidata.ucar.edu/thredds/catalog/nexrad/level2/S3/catalog.xml')
rs=''
